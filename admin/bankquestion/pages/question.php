  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main" style="height: calc(100vh - 330px);">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link text-white active bg-gradient-primary" href="./question.php">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
              <i class="material-icons opacity-10">table_view</i>
            </div>
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <h6 class="font-weight-bolder mb-0">Ngân hàng câu hỏi</h6>
        </nav>
      </div>
    </nav>
    <!-- End Navbar -->
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Ngân hàng câu hỏi</h6>
              </div>
            </div>
            <div class="card-body px-0 pb-2">
              <div class="d-flex justify-content-around">
                <div class="w-25">
                  <button id="btnquestion" type="button" class="btn btn-success" style="margin-right: 10%;">Thêm câu hỏi</button>
                </div>
              </div>
...
<script type="text/javascript">
  ...
  $('#btnquestion').click(function() {
    ...
    $('#category_selected').val('Lựa chọn loại câu hỏi');
    $('#media_selected').val('Lựa chọn phương tiện');
  ...
  function GetDetail(id) { //hàm lấy câu hỏi dựa vào id câu hỏi
    $.ajax({
      ...
      success: function(data) {
        ...
        $('#modalquestion').modal('show');
        $('#texaQuestion').val(q['name_question']);
        $('#texaOptionA').val(q['option1']);
        $('#texaOptionB').val(q['option2']);
        $('#texaOptionC').val(q['option3']);
        $('#texaOptionD').val(q['option4']);
        $('#texaAnswer').val(q['answer']);
        switch (q['option_correct']) {
          case 'A':
