    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <h6 class="font-weight-bolder mb-0">Bài tập</h6>
        </nav>
      </div>
    </nav>
    <!-- End Navbar -->
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Đề luyện tập</h6>
              </div>
            </div>
            <div class="card-body px-0 pb-2">
              <div class="d-flex justify-content-around">
                <div class="w-25">
                  <button id="btnExam" type="button" class="btn btn-success" style="margin-right: 10%;">Thêm bài tập</button>
                </div>
              </div>
...
<script type="text/javascript">
  ...
  function GetView(id) { //hàm lấy đề thi dựa vào id đề thi
    $.ajax({
      url: './detailtest.php', //chỉ hướng dẫn tới file detail.php để lấy thông tin đề thi
      type: 'get',
      data: {
        id: id //truyền tham số có giá trị bằng giá trị của id đề thi
      },
      success: function(data) {
        var w = window.location.href = './detailtest.php?id=' + id;
  ...
  function GetDetail(id) { //hàm lấy đề thi dựa vào id đề thi
    $.ajax({
      ...
      success: function(data) {
        ...
        $('#modalexam').modal('show');
        $('#nameExam').val(q['name_Test']);
        $('#categoryExam_selected').val(q['id_categoryquestion']);
      }
    });
  }
